...
registerBlockType( 'lez-library/private-note', {
	...
	category: 'lezwatch',
	description: 'Private notes, only seen by logged in users. It will be stripped from all published pages.',
		...
		return (
			<Fragment>
				<div className={ `${ className } alert alert-warning` }>
					<InnerBlocks
						template={[
							['core/paragraph', { content: 'All content in this block will be invisible to non-logged-in visitors (delete this and replace it).' }],
						]}
						templateLock={ false }
					/>
				</div>
